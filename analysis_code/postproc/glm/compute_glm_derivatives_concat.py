...
-----------------------------------------------------------------------------------------
Exemple:
python compute_glm_derivatives_concat.py /scratch/mszinte/data RetinoMaps sub-02 327
-----------------------------------------------------------------------------------------
...
glm_fit_fns = []
for format_, extension in zip(formats, extensions):
    list_ = glob.glob("{}/{}/derivatives/pp_data/{}/{}/glm/glm_fit/*fit_concat*.{}".format(main_dir, 
                                                                                    project_dir, 
                                                                                    subject, 
                                                                                    format_, 
                                                                                    extension))
...
for glm_files in glm_files_list:
    ...
    # export final map
    if hemi:
        final_fn = "{}/{}/derivatives/pp_data/{}/fsnative/glm/glm_derivatives/{}_task-eyes-mvt_{}_space-fsnative_dct_glm-significant_map_concat.func.gii".format(
            main_dir, project_dir, subject, subject, hemi)
        
    else:
        final_fn = "{}/{}/derivatives/pp_data/{}/170k/glm/glm_derivatives/{}_task-eyes-mvt_space-fsLR_den-170k_dct_glm-significant_map_concat.dtseries.nii".format(
