#!/bin/bash
#-----------------------------------------------------------------------------------------
# compute_css_stats.sh
# -----------------------------------------------------------------------------------------
# Goal of the script:
# Launch across subjects the function compute_css_stats.py
# -----------------------------------------------------------------------------------------
# Input(s):
...
# 2. run shell command
# >> sh compute_css_stats.sh [code directory] [project name] [main directory]
# -----------------------------------------------------------------------------------------
# Exemple:
# cd ~/projects/RetinoMaps/analysis_code/postproc/prf/postfit
# sh compute_css_stats.sh ~/projects RetinoMaps /scratch/mszinte/data
# -----------------------------------------------------------------------------------------
...
do
    echo "Processing compute_css_stats.py for: $subject"
    python compute_css_stats.py "$data_path" "$project_name" "$subject" 327
done
