...
data_types = ['css-prf_derivatives', 'css-prf_derivatives_group']
...
for format_, extension in zip(formats, extensions):
    for data_type in data_types:
        ...
        for subject in subjects:
            ...
            # Construct path to the directory containing subject's data
            ...
            # Calculate mean data for each subject
            ...
            # Prepare categorical data and concatenate mean data
        ...
        data_all.to_csv('{}/group_css-prf_derivatives_{}.tsv'.format(tsv_all_dir, data_type), sep="\t", na_rep='NaN', index=False)
        data_mean_all.to_csv('{}/group_css-prf_derivatives_{}.tsv'.format(tsv_all_dir, data_type), sep="\t", na_rep='NaN', index=False)
...
#         data = pd.read_table('{}/{}_css-prf_derivatives'.format(tsv_dir,subject))
...
#     data_all.to_csv('{}/group_css-prf_derivatives.tsv'.format(tsv_all_dir), sep="\t", na_rep='NaN',index=False)
#     data_mean_all.to_csv('{}/group_css-prf_derivatives.tsv'.format(tsv_all_dir), sep="\t", na_rep='NaN',index=False)

# #  Make group for mmp rois grouped    
# for format_, extension in zip(formats, extensions):
#     # load subjects tsv and concatenate them
#     data_all = pd.DataFrame()
#     data_mean_all = pd.DataFrame() 
...
#         data = pd.read_table('{}/{}_css-prf_derivatives_group'.format(tsv_dir,subject))
#         data.loc[(data.prf_ecc < ecc_th[0]) | (data.prf_ecc > ecc_th[1]) | (data.prf_size < size_th[0]) | (data.prf_size > size_th[1]) | (data.prf_n < n_th[0]) | (data.prf_n > n_th[1]) | (data.prf_loo_r2 <= rsq_th[0])] = np.nan
...
#     data_all.to_csv('{}/group_css-prf_derivatives_group.tsv'.format(tsv_all_dir), sep="\t", na_rep='NaN',index=False)
#     data_mean_all.to_csv('{}/group_css-prf_derivatives_group.tsv'.format(tsv_all_dir), sep="\t", na_rep='NaN',index=False)   
