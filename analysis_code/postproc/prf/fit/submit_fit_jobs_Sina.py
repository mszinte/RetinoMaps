...
-----------------------------------------------------------------------------------------
Exemple:
python submit_fit_jobs_Sina.py /scratch/mszinte/data RetinoMaps sub-02 327
-----------------------------------------------------------------------------------------
...
# pp_avg_fns_HCP = glob.glob("{}/{}/func/fmriprep_dct_avg/HCP_170k/*_pRF_*_avg*.dtseries.nii".format(pp_dir,subject))

pp_avg_fns_HCP = ['{}/{}/func/fmriprep_dct_avg/HCP_170k/sub-02_task-pRF_fmriprep_dct_avg_bold.dtseries.nii'.format(pp_dir,subject)]
...
for fit_num, pp_avg_fn in enumerate(pp_avg_fns_HCP):
    ...
    if os.path.isfile(fit_fn) and os.path.getsize(fit_fn) != 0:
...
#SBATCH -J {sub}_fit_{fit_num}\n\n""".format(
        nb_procs=nb_procs, log_dir=prf_logs_dir, sub=subject, fit_num=fit_num)

    # define fit cmd
    fit_cmd = "python prf_fit_surf_Sina.py {} {} {} {} {} {}".format(
