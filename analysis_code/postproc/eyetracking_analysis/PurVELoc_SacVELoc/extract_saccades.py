...
python extract_saccades.py sub-01 SacVELoc ses-01
-----------------------------------------------------------------------------------------
"""
#%%
# Stop warnings
...
polar_ang = np.deg2rad(np.arange(analysis_info['ang_start_sac'],360,analysis_info['ang_steps']))
...
h5_filename = "{file_dir}/{sub}_{ses}_task-{task}_eyedata.h5".format(file_dir = file_dir_save, sub = subject, ses = session, task = task)
...
dir_sequence = np.array(h5_file['dir_sequence'])
		...
		for trial in trials:
			...
			if (seq_type[sequence] == 0) :
				...
				sac_pos_x, sac_pos_y = np.round(np.cos(polar_ang[np.mod(trial_with_sac+1,4)])*amp_sac,decimals=3),\
								   	   np.round(np.sin(polar_ang[np.mod(trial_with_sac+1,4)])*amp_sac,decimals=3)
			else: # cw
				polar_ang_flip = np.flip(polar_ang, axis=0)
				fix_pos_x, fix_pos_y = np.round(np.cos(polar_ang_flip[np.mod(trial_with_sac,4)])*amp_sac,decimals=3),\
								   	   np.round(np.sin(polar_ang_flip[np.mod(trial_with_sac,4)])*amp_sac,decimals=3)
				sac_pos_x, sac_pos_y = np.round(np.cos(polar_ang_flip[np.mod(trial_with_sac+1,4)])*amp_sac,decimals=3),\
								   	   np.round(np.sin(polar_ang_flip[np.mod(trial_with_sac+1,4)])*amp_sac,decimals=3)
