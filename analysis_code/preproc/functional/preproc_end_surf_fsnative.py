...
for session in sessions :
    ...
    for hemi in hemis :
        ...
        for func_fn_hemi in fmriprep_func_hemi_fns :
            ...
            # Load data
            surf_img_hemi, surf_data_hemi = load_gifti_image(gii_fn=func_fn_hemi)
            
            # High pass filtering Left hemisphere
            n_tr_hemi = surf_data_hemi.shape[0]
            ft_hemi = np.linspace(0.5 * TR, (n_tr_hemi + 0.5) * TR, n_tr_hemi, endpoint=False)
        ...
        for task in tasks:
            ...
            # preproc tasks runs
            preproc_files_hemi = glob.glob("{}/*_task-{}_*_hemi-{}_space-fsnative_bold_{}.func.gii".format(
                pp_data_func_dir, task,hemi, high_pass_type))
            ...
            # output files 
            cor_file_hemi = "{}/{}_task-{}_hemi-{}_fmriprep_{}_correlations_bold.func.gii".format(
                corr_dir, subject, task, hemi, high_pass_type)

            # load preproc files to have meta and header
            preproc_img_hemi, preproc_data_hemi = load_gifti_image(
                gii_fn=preproc_files_hemi[0])
            ...
            for combi_hemi in combis_hemi:
                ...
                a_img_hemi, a_data_hemi = load_gifti_image(gii_fn=combi_hemi[0])
                b_img_hemi, b_data_hemi = load_gifti_image(gii_fn=combi_hemi[1])
                
                for vertice_hemi in range(a_data_hemi.shape[1]):
                    corr_hemi, _ = stats.pearsonr(a_data_hemi[:, vertice_hemi], 
                                                  b_data_hemi[:, vertice_hemi])
            ...
            # avg files 
            avg_file_hemi = "{}/{}_task-{}_hemi-{}_fmriprep_{}_avg_bold.func.gii".format(
            ...
            for preproc_file_hemi in preproc_files_hemi:
                ...
                # Load data
                preproc_img_hemi, preproc_data_hemi = load_gifti_image(gii_fn=preproc_file_hemi)
                
                # Averaging 
                data_avg_hemi += preproc_data_hemi/len(preproc_files_hemi)

            # export averaging data
            avg_img_hemi = make_gifti_image(preproc_img_hemi,data_avg_hemi)
            ...
            for loo_num, avg_runs in enumerate(combi_hemi):
                ...
                preproc_img_hemi, preproc_data_hemi = load_gifti_image(gii_fn=preproc_files_hemi[0])
                ...
                loo_avg_img_hemi = make_gifti_image(preproc_img_hemi,data_loo_avg_hemi)
                ...
                for loo in preproc_files_hemi:
                    if loo not in avg_runs:
                        loo_file_hemi =  "{}/{}_task-{}_hemi-{}_fmriprep_{}_loo-{}_bold.func.gii".format(
                            loo_avg_dir, subject,task,hemi,high_pass_type, loo_num+1)
