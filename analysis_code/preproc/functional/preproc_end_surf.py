...
fmriprep_dir = file_path
fmriprep_func_RH_fns = glob.glob("{}/*_hemi-R_space-fsaverage_bold.func.gii".format(fmriprep_dir))
fmriprep_func_LH_fns = glob.glob("{}/*_hemi-L_space-fsaverage_bold.func.gii".format(fmriprep_dir))
...
print("Right hemisphere is processing...")
for func_fn_R in fmriprep_func_RH_fns :   
    
    # Right hemisphere 
    surface_data_R = nb.load(func_fn_R)
    
    
    
    surf_data_R = [x.data for x in surface_data_R.darrays]
    surf_data_R = np.reshape(surf_data_R,(len(surf_data_R[0]),len(surf_data_R)))
    

    n_vol = surf_data_R.shape[0]
    ft = np.linspace(0.5 * TR, (n_vol + 0.5) * TR, n_vol, endpoint=False)
    hp_set = _cosine_drift(high_pass_threshold, ft)
    surf_data_R = signal.clean(surf_data_R, detrend=False, standardize=True, confounds=hp_set)
